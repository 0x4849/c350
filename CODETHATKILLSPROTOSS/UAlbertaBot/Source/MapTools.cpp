    ...
    for (BWTA::BaseLocation * base : BWTA::getBaseLocations())
    {
        ...
        if (!base->isMineralOnly() && !(base == BWTA::getStartLocation(player)))
        {
			...
			if ((!UnitUtil::GetAllUnitCount(BWAPI::UnitTypes::Zerg_Spire) > 0)
				&& (UnitUtil::GetAllUnitCount(BWAPI::UnitTypes::Zerg_Hatchery) >= 2))
			{
				//prevent expanding at a location that has nearby enemies
				/*
				std::vector<UnitInfo> unitInfo;
				InformationManager::Instance().getNearbyForce(unitInfo, BWAPI::Position(tile), BWAPI::Broodwar->enemy(), 350);
				if (!unitInfo.empty())
				{
					BWAPI::Broodwar->printf("Enemies too near expansion location, try somewhere else");
					continue;
				}*/
