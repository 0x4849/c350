...
void BuildingManager::update()
{
	...
	if (createdHatchery)
	{
		bool isMove = sunkenUnit->isMoving();
		bool isIdle = sunkenUnit->isIdle();
		double myDist = sunkenUnit->getDistance(BWAPI::Position(createdHatcheriesVector[0]));
		bool isBuilding = sunkenUnit->isMorphing();
		BWAPI::Broodwar->printf("X: %d Y: %d VS : X : %d, Y: %d MOVING : %d IDLE : %d DIST : %f\n", sunkenUnit->getTilePosition().x, sunkenUnit->getTilePosition().y, createdHatcheriesVector[0].x, createdHatcheriesVector[0].y, isMove, isIdle, myDist);

		if (sunkenUnit->getDistance(BWAPI::Position(createdHatcheriesVector[0])) <= 60 && startedPool && !isCreepStarted())
		{
			//std::set<BWAPI::TilePosition>::iterator it = buildableSunkenTilePositions.begin(), end = buildableSunkenTilePositions.end();
		
			//BWAPI::TilePosition blah = getSunkenPosition();

			/*
			if (blah != BWAPI::TilePositions::None && !makeNext)
			{
			...
			*/
			...
			//BWAPI::TilePosition FUCKYOU = createdHatcheriesVector[0];


			//BWAPI::UnitType sunk = BWAPI::UnitTypes::Zerg_Creep_Colony;
			//sunkenUnit->move(BWAPI::Position(FUCKYOU));
			//sunkenUnit->build(sunk,blah);
			/*
			for (auto x : buildableSunkenTilePositions)
			{
				BWAPI::Broodwar->printf("Trying to build at %d %d \n", x.x, x.y);

				BWAPI::UnitType sunk = BWAPI::UnitTypes::Zerg_Creep_Colony;
				Building b(sunk, x);
				

				if (createdBuilding.find(x) == createdBuilding.end())
				{
					
					//sunkenUnit->move(BWAPI::Position(x));
					sunkenUnit->build(sunk, x);
					BWAPI::Broodwar->printf("Built at %d %d \n", x.x, x.y);
					break;
				}
				else
				{
					buildableSunkenTilePositions.erase(x);
				}
	...
	//StrategyManager::Instance().shouldBuildSunkens();
...
void BuildingManager::assignWorkersToUnassignedBuildings()
{
	...
	for (Building & b : _buildings)
	{
		...
		//if (!isCreepStarted() && createdHatcheriesVector.size() >= 1 && startedPool)
		///{
		...
		//BWAPI::Unit workerToAssign = WorkerManager::Instance().getBuilder(b);
			...
			/*
			BWAPI::TilePosition myPOS = b.finalPosition;
			bool blah = isCreepStarted();
			bool blah2 = createdHatcheriesVector.size() >= 1;
			bool blah3 = createdSunkenSet.find(b.finalPosition) != createdSunkenSet.end();
			bool blah4 = startedPool;
			*/
	...
	for (auto & b : _buildings)
	{
		...
		if (!b.builderUnit->isConstructing())
		{
			...
			else
			{
				...
				{
					BWAPI::TilePosition sunkPos;
					//if (!isCreepStarted() && createdSunkenSet.size() >= 1)
					//{
					//	sunkPos = createdSunkenVector[0];
					//}
					//else
					//{
					...
					//BWAPI::TilePosition builderPos = b.builderUnit->getTilePosition();
					//BWAPI::Broodwar->printf("%i %i\n", builderPos.x, builderPos.y);
	...
	for (auto & b : _buildings)
	{
		...
		if (b.buildingUnit->isCompleted())
		{
			if (BWAPI::Broodwar->self()->getRace() == BWAPI::Races::Zerg)
			{
				...
				{
					MetaType type(BWAPI::UnitTypes::Zerg_Sunken_Colony);
					//ProductionManager::Instance()._queue.queue(type, true);
		...
		{
			if (b.type == 131 && b.buildingUnit->getHitPoints() >= 1150 && !transferredDrones)
			{
				...
				BWAPI::TilePosition tempPosition;
				tempPosition.x = createdHatcheriesVector[0].x+1;
				tempPosition.y = createdHatcheriesVector[0].y+0;

				BWAPI::Unit workerToAssign = WorkerManager::Instance().getMoveWorker(BWAPI::Position(tempPosition));
				WorkerManager::Instance().setMoveWorker(75, 0, BWAPI::Position(tempPosition));

				MetaType type(BWAPI::UnitTypes::Zerg_Creep_Colony);
				ProductionManager::Instance()._queue.queueAsHighestPriority(type, true);
				//BWAPI::Unit workerToAssign2 = WorkerManager::Instance().getMoveWorker(BWAPI::Position(tempPosition));

				//WorkerManager::Instance().setMoveWorker(0, 0, BWAPI::Position(tempPosition));
				...
				*/
				...
				// Get the set of mineral patches closest to BWTA::Location of the hatchery(it will return like 8 mineral patches usually in the set)
				const BWAPI::Unitset mineralSet = myLocation->getGeysers();
				...
				//workerToAssign->rightClick(myMineral);
			...
			if (b.type == 131 && b.buildingUnit->getHitPoints() >= 1200 && !sentFirstDroneForSunken)
			{
