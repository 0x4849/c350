...
{
	//BWAPI::Broodwar->printf("Entered getNExtExpansion\n");
    ...
    {
        // if the base has gas
		if (!base->isMineralOnly() && !(base == BWTA::getStartLocation(player)))// && (BuildingManager::Instance().closestBaseSet.find(base->getTilePosition()) == BuildingManager::Instance().closestBaseSet.end()))
		{
    ...
    if (closestBase)
    {
		//closestBaseSet.insert(closestBase);
