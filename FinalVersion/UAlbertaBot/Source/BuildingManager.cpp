...
//Get a spire position in the main
BWAPI::TilePosition BuildingManager::getSpirePosition()
{
	const std::vector<BWAPI::TilePosition>  & tempTiles = MapTools::Instance().getClosestTilesTo(BWAPI::Position(BWAPI::Broodwar->self()->getStartLocation()));
...
BWAPI::TilePosition BuildingManager::getBuildingLocation(const Building & b)
{
	...
	else if ((Config::Strategy::StrategyName == Config::Strategy::AgainstProtossStrategyName) && (b.type.isResourceDepot()) && (!b.isMacro))
	{
		BWAPI::TilePosition tile = MapTools::Instance().getNextExpansion();
		return tile;
	}
