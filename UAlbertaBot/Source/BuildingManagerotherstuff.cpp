...
BWAPI::TilePosition BuildingManager::getSunkenPosition(BWAPI::Unit myBuilder)
{
	...
	if (createdHatcheriesSet.size() >= 1)
	{
		...
		*/
		...
		const BWAPI::Position x = BWAPI::Position(createdHatcheriesVector[0]);
		const std::vector<BWAPI::TilePosition>  & tempTiles = MapTools::Instance().getClosestTilesTo(x);
		//const std::vector<BWAPI::TilePosition>  & tempTiles2 = MapTools::Instance().getClosestTilesTo(BWAPI::Position(ourRampPosition));
		const std::vector<BWAPI::TilePosition>  & tempTiles3 = MapTools::Instance().getClosestTilesTo(BWAPI::Position(BWAPI::Broodwar->self()->getStartLocation()));
		//const std::vector<BWAPI::TilePosition>  & tempTiles4 = MapTools::Instance().getClosestTilesTo(BWAPI::Position(createdHatcheriesVector[0]));
		
		BWAPI::Broodwar->printf("The size is %d\n", tempTiles.size());
		...
		for (auto &tile : tempTiles3)
		{
			//BWAPI::Broodwar->printf("INSIDE LOOP\n");
			const BWAPI::TilePosition & tp = tile;
			if (buildable(tp.x, tp.y, tp))
			{
				buildableSunkenTilePositions.insert(tp);
			}
		...
		{
			BWAPI::Broodwar->printf("Returning non zero sunken\n");
			std::set<BWAPI::TilePosition>::iterator it = buildableSunkenTilePositions.begin();
			return *it;
		}
		else
	...
	for (auto & b : _buildings)
	{
		...
		if (b.buildingUnit->isCompleted())
		{
			...
			if (b.buildingUnit->getType() == BWAPI::UnitTypes::Zerg_Hatchery && b.finalPosition == createdHatcheriesVector[0] && canBuildTrigger)
			{
				...
				for (BWTA::BaseLocation *p : locations) {
					BWAPI::TilePosition z = p->getTilePosition();
					if (z == b.buildingUnit->getTilePosition()){
						// This is the BWTA::Location of the first hatchery.
						myLocation = p;
				...
				for (BWAPI::Unit p : mineralSet)
				{
					if (counter1 == 4)
					{
						ourMineralPosition = p->getTilePosition();
						break;
					}
					counter1++;
				}

				mineralToHatcheryDistance = BWTA::getGroundDistance(ourMineralPosition, createdHatcheriesVector[0]);
				//Get all geysers near the expansion -- it should only return 1 for every map we play..
				const BWAPI::Unitset gasSet = myLocation->getGeysers();
				int gasX, gasY;
				for (BWAPI::Unit p : gasSet)
				{
					naturalGas = p;
					// Calculate the difference between Geyser.x- ExpansionHatchery.x and store it in gasX
					gasX = p->getTilePosition().x - createdHatcheriesVector[0].x;
					ourGasPosition = p->getTilePosition();
					// Calculate the difference between Geyser.y- ExpansionHatchery.y and store it in gasY
					gasY = p->getTilePosition().y - createdHatcheriesVector[0].y;
					break;

				}
				gasToHatcheryDistance = BWTA::getGroundDistance(ourGasPosition, createdHatcheriesVector[0]);
				/*
				bool useGasX = false;
				bool useGasY = false;
				bool useMinX = false;
