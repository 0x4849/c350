...
void Squad::update()
{
	...
	if (Config::Debug::DrawSquadInfo && (_order.getType() == SquadOrderTypes::Attack || _order.getType() == SquadOrderTypes::Harass))
	{
	...
	//TOMMY
	if (needToRegroup && (_order.getType() != SquadOrderTypes::Confuse) && (_order.getType() != SquadOrderTypes::Harass))
	{
...
bool Squad::needsToRegroup()
{
	...
	if (BWAPI::Broodwar->enemy()->getRace() == BWAPI::Races::Protoss)
	{
		...
		for (auto &unit : _units)
		{
			...
			for (auto &psiStorm : psi)
			{
				...
				dist = UnitUtil::GetAirDistance(unitPos, psiPos);
				if (dist < 32)
				{
					stormed = true;
				}
	...
	if ((_units.empty()) || ((_order.getType() != SquadOrderTypes::Attack) && (_order.getType() != SquadOrderTypes::Harass)))
	{
	...
	if (stormed)
	{
		BWAPI::Broodwar->printf("Run from storms!");
	}
...
bool Squad::unitNearEnemy(BWAPI::Unit unit)
{
	...
	if (BWAPI::Broodwar->enemy()->getRace() == BWAPI::Races::Protoss)
	{
		...
		for (auto &psiStorm : psi)
		{
			...
			{
				stormed = true;
				BWAPI::Broodwar->printf("Regroup out of storm range");
				break;
			}
		}
	}

	return (enemyNear.size() > 0) || (stormed);
}
