...
BWAPI::TilePosition BuildingManager::getSunkenPosition()
{
	...
	if (createdHatcheriesSet.size() >= 1)
	{
		...
		BWTA::Chokepoint *p;
		p = BWTA::getNearestChokepoint(hatchPosition);



		BWAPI::Position centerOfChoke = p->getCenter();
		...
		for (BWAPI::Unit p : mineralSet)
		{
			if (counter3 == 0)
			{
				...
				for (BWAPI::Unit u : BWAPI::Broodwar->self()->getUnits())
				{
					if (u->getType() == 42)
					{
						u->move(BWAPI::Position(p->getTilePosition()));
						break;
			...
			//BWAPI::Broodwar->printf("The test is x: %i, y: %i\n hatchPosition x is %i, hatchPosition y is %i\n", p->getTilePosition().x, p->getTilePosition().y, hatchPosition.x, hatchPosition.y);
			counter3++;
		}
		...
		while ((!BWAPI::Broodwar->hasCreep(sunkPosition) || !BuildingPlacer::Instance().canBuildHere(sunkPosition, b) || createdBuilding.find(sunkPosition) != createdBuilding.end()) && counter <= 7)
		{
			if (counter == 8)
			{
				break;
			}
			if (incrementDecrement[counter] && outercounter <= 15)
			{
				...
				//Decrease x Dont Change y
				else if (counter == 5)
				{
					beginX -= 1;
				}
				//Don't change X Increase y
				else if (counter == 6)
				{
					beginY += 1;
				}
				//Don't change X Decrease y
				else if (counter == 7)
				{
					beginY -= 1;
				}
		...
		/*
		double2 direc = centerOfChoke - hatchPositionBWP;
		...
		*/
		...
		//if (createdSunkenSet.size() == 0)
		//{
		/*

		int newX = hatchPosition.x + int(direcNormal.x * 32);
		int newY = hatchPosition.y + int(direcNormal.y * 32);
		hatchPositionBWP = BWAPI::Position(newX, newY);
		*/
		/*
		int newx, newy;
		if (decreaseX)
		{
			newx = hatchPosition.x - 1;
		}
		else
		{
			newx = hatchPosition.x + 1;
		...
		BWAPI::TilePosition myTest;
		myTest.x = hatchPosition.x - 1;
		myTest.y = hatchPosition.y;

		bool test;
		test = BWAPI::Broodwar->hasCreep(sunkPosition);
		bool test2;
		test2 = BWAPI::Broodwar->hasCreep(myTest);
		bool test3;
		test3 = BWAPI::Broodwar->hasCreep(hatchPosition);
		BWAPI::Broodwar->printf("The test is %i", test);
		...
		BWAPI::Broodwar->printf("The test is %i", test4);
		/*
		...
		while (!BWAPI::Broodwar->hasCreep(sunkPosition) || !BuildingPlacer::Instance().canBuildHere(sunkPosition, b))
		{	
			if (counter == 0)
			{
				beginX += 1;
	...
	//}
	/*
	if (createdSunkenSet.size() >= 1 )
	{
	int myIndex = createdSunkenVector.size() - 1;
	int newX = createdSunkenVector[myIndex].x;
	int newY = createdSunkenVector[myIndex].y;

	BWAPI::Position test = BWAPI::Position(newX, newY);
	sunkPosition = BWAPI::TilePosition(newX, newY);

	while (!BWAPI::Broodwar->hasCreep(sunkPosition))
	{

	int newY = hatchPosition.y + int(direcNormal.y * 32);
	sunkPosition = BWAPI::Position(newX, newY);

	sunkPosition = BWAPI::TilePosition(hatchPositionBWP.x, hatchPositionBWP.y);
	while (!BWAPI::Broodwar->hasCreep(sunkPosition))
	{
	int newX = hatchPosition.x + int(direcNormal.x * 32);
	int newY = hatchPosition.y + int(direcNormal.y * 32);
