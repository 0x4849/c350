...
void RangedManager::assignTargetsOld(const BWAPI::Unitset & targets)
{
    ...
    for (auto & rangedUnit : rangedUnits)
	{
		...
		if (order.getType() == SquadOrderTypes::Attack || order.getType() == SquadOrderTypes::Defend || order.getType() == SquadOrderTypes::Harass)
        {
			...
			if (!rangedUnitTargets.empty())
			{
				...
				if (order.getType() == SquadOrderTypes::Harass)
				{
					...
					sim.setCombatUnits(rangedUnit->getPosition(), 400);
...
//TOMMY
void RangedManager::mutaFlee(BWAPI::Unit unit)
{
	Micro::SmartMove(unit, getNearestCorner(unit));
}

BWAPI::Position RangedManager::getNearestCorner(BWAPI::Unit unit)
{
	BWAPI::TilePosition unitPos = unit->getTilePosition();
	BWAPI::Point<int, 32> corner1(BWAPI::Broodwar->mapWidth()-1, 1);
	BWAPI::Point<int, 32> corner2(1, BWAPI::Broodwar->mapHeight()-1);
	BWAPI::Point<int, 32> corner3(1, 1);
	BWAPI::Point<int, 32> corner4(BWAPI::Broodwar->mapWidth()-1, BWAPI::Broodwar->mapHeight()-1);
	double distTo1 = UnitUtil::GetAirDistance(unitPos, corner1);
	double distTo2 = UnitUtil::GetAirDistance(unitPos, corner2);
	double distTo3 = UnitUtil::GetAirDistance(unitPos, corner3);
	double distTo4 = UnitUtil::GetAirDistance(unitPos, corner4);
	std::vector<double> dists;
	dists.push_back(distTo1);
	dists.push_back(distTo2);
	dists.push_back(distTo3);
	dists.push_back(distTo4);
	std::sort(dists.begin(), dists.end());
	double shortestDist = dists.front();
	if (shortestDist == distTo1)
	{
		return BWAPI::Position(corner1);
	}
	else if (shortestDist == distTo2)
	{
		return BWAPI::Position(corner2);
	}
	else if (shortestDist == distTo3)
	{
		return BWAPI::Position(corner3);
	}
	else if (shortestDist == distTo4)
	{
		return BWAPI::Position(corner4);
	}
	else
	{
		BWAPI::Broodwar->printf("Did not find corner to flee to");
