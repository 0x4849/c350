		...
		if (!b.builderUnit->isConstructing())
		{
			...
			else
			{
				...
				if (b.type == BWAPI::UnitTypes::Zerg_Hatchery)
				{
					...
					{
						if (createdHatcheriesVector.size() >= 1 && shouldIExpand)
						{
							BWAPI::Broodwar->printf("Inside of finding location for expand.\n");
							BWAPI::TilePosition myExp = getNextExpandLocation();
							BWAPI::Broodwar->printf("Finished Inside of finding location for expand.\n");
							if (myExp != BWAPI::TilePositions::None)
							{

								BWAPI::Broodwar->printf("ASsigning final position for expand..\n");
								b.finalPosition = myExp;
							}
							else
							{
								BWAPI::Broodwar->printf("We returned none as the expansin location. Oh no!\n");
							}
						}
					}
...
void BuildingManager::checkForCompletedBuildings()
{
	...
	for (auto & b : _buildings)
	{
		...
		if (b.buildingUnit->isCompleted())
		{
			...
			else if (b.buildingUnit->getType() == BWAPI::UnitTypes::Zerg_Hatchery && b.buildingUnit->getTilePosition() != BWAPI::Broodwar->self()->getStartLocation() && b.buildingUnit->getTilePosition() != createdHatcheriesVector[0] && isBaseLocation(b.buildingUnit->getTilePosition()))
			{
