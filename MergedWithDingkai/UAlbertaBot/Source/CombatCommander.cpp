...
void CombatCommander::initializeSquads()
{
	...
	SquadOrder harassOrder(SquadOrderTypes::Harass, getMainHarassLocation(), 1000, "Harass");
	_squadData.addSquad("Harass", Squad("Harass", harassOrder, HarassPriority));
...
{
	// code to turn off harass
	return;

	if (!_squadData.squadExists("Harass"))
	{
		return;
	}
	...
	for (auto & unit : _combatUnits)
	{
		if ((_squadData.canAssignUnitToSquad(unit, harassSquad)) && (unit->getType() == BWAPI::UnitTypes::Zerg_Mutalisk))
		{
...
void CombatCommander::updateDefenseSquadUnits(Squad & defenseSquad, const size_t & flyingDefendersNeeded, const size_t & groundDefendersNeeded)
{
    ...
    while (groundDefendersNeeded > groundDefendersInSquad + groundDefendersAdded)
    {
        ...
        {
			//BWAPI::Broodwar->printf("could not find defender");
            break;
        }
    }
}
...
//TOMMY
BWAPI::Position CombatCommander::getMainHarassLocation()
{
	BWTA::BaseLocation * enemyBaseLocation = InformationManager::Instance().getMainBaseLocation(BWAPI::Broodwar->enemy());
