...
BWAPI::TilePosition BuildingManager::getSunkenPosition()
{
	...
	if (createdHatcheriesSet.size() >= 1)
	{
		...
		while (buildableSunkenTilePositions.size() >= 1)
		{
			...
			{
				if (BWAPI::Broodwar->hasCreep(mySunkPosition) && BuildingPlacer::Instance().canBuildHereWithSpace(mySunkPosition, z) && createdBuilding.find(mySunkPosition) == createdBuilding.end() && stuffBlocking.find(mySunkPosition) == stuffBlocking.end())
				{
					return *it;
				}
				else
		...
		for (int l = 0; l < 30; l++)
		{
			...
			*/
			...
			{
				if (BWAPI::Broodwar->hasCreep(sunkPosition) && BuildingPlacer::Instance().canBuildHereWithSpace(sunkPosition, b) && stuffBlocking.find(sunkPosition) == stuffBlocking.end() && createdBuilding.find(sunkPosition) == createdBuilding.end())
				{
					buildableSunkenTilePositions.insert(sunkPosition);
				}
			}
